...
class AttentionLayer(nn.Module):
    ...
    def forward(self, left_inp, right_inp):
        ...
        ###
        # print(f'Network.py L110: {sigged.shape}')
        # print(f'Network.py L111: {right_inp.shape}')
        target_size = right_inp.shape[2:]
        sigged = F.interpolate(sigged, size=target_size, mode='bilinear')
        ###
...
class VGGScaleClampEdgeDecoderBatchNorm(nn.Module):
    ...
    def forward(self, encoderDict):
        ...
        ###
        target_size = edge_deconvs0_list[0].shape[2:]

        conv_30 = F.interpolate(encoderDict['conv30'], size=target_size, mode='bilinear')
        cross_30 = F.interpolate(encoderDict['cross30'], size=target_size, mode='bilinear')

        edge_deconvs0_list.append(conv_30)
        edge_deconvs0_list.append(cross_30)

        # print(len(edge_deconvs0_list))
        # for i in edge_deconvs0_list:
        #     print(i.shape)
        ...
        ###
        target_size = edge_deconvs1_list[0].shape[2:]

        conv_20 = F.interpolate(encoderDict['conv20'], size=target_size, mode='bilinear')
        cross_20 = F.interpolate(encoderDict['cross20'], size=target_size, mode='bilinear')

        edge_deconvs1_list.append(conv_20)
        edge_deconvs1_list.append(cross_20)

        # print(len(edge_deconvs1_list))
        # for i in edge_deconvs1_list:
        #     print(i.shape)
        ...
        ###
        target_size = edge_deconvs2_list[0].shape[2:]

        conv_10 = F.interpolate(encoderDict['conv10'], size=target_size, mode='bilinear')
        cross_10 = F.interpolate(encoderDict['cross10'], size=target_size, mode='bilinear')

        edge_deconvs2_list.append(conv_10)
        edge_deconvs2_list.append(cross_10)

        # print(len(edge_deconvs2_list))
        # for i in edge_deconvs2_list:
        #     print(i.shape)
        ...
        ###
        target_size = edge_deconvs3_list[0].shape[2:]

        conv_00 = F.interpolate(encoderDict['conv00'], size=target_size, mode='bilinear')
        cross_00 = F.interpolate(encoderDict['cross00'], size=target_size, mode='bilinear')

        edge_deconvs3_list.append(conv_00)
        edge_deconvs3_list.append(cross_00)

        # print(len(edge_deconvs3_list))
        # for i in edge_deconvs3_list:
        #     print(i.shape)
...
class VGGUnrefinedDecoderBatchNorm(nn.Module):
    ...
    def forward(self, encoderDict, edgeDict):
        ...
        ###
        target_size = unrefined_deconvs0_list[0].shape[2:]

        conv_30 = F.interpolate(encoderDict['conv30'], size=target_size, mode='bilinear')

        unrefined_deconvs0_list.append(conv_30)

        # print(len(unrefined_deconvs0_list))
        # for i in unrefined_deconvs0_list:
        #     print(i.shape)
        ...
        ###
        target_size = unrefined_deconvs1_list[0].shape[2:]

        conv_20 = F.interpolate(encoderDict['conv20'], size=target_size, mode='bilinear')

        unrefined_deconvs1_list.append(conv_20)

        # print(len(unrefined_deconvs1_list)) # 2
        # for i in unrefined_deconvs1_list:
        #     print(i.shape)

        unrefined_deconvs1_comb = torch.cat(unrefined_deconvs1_list, 1)
        ###

        # print(len(unrefined_deconvs1_comb)) # 1
        # print(unrefined_deconvs1_comb.shape)
        unrefined_deconvs2_list = []
        store_unrefined_deconvs2 = [deconvs2(unrefined_deconvs1_comb) for deconvs2 in
                              self.unrefined_deconvs2]

        # print(len(store_unrefined_deconvs2)) # 2
        # for i in store_unrefined_deconvs2:
        #     print(i.shape)
        ...
        ###
        target_size = unrefined_deconvs2_list[0].shape[2:]

        conv_10 = F.interpolate(encoderDict['conv10'], size=target_size, mode='bilinear')

        unrefined_deconvs2_list.append(conv_10)

        # print(len(unrefined_deconvs2_list)) # 3
        # for i in unrefined_deconvs2_list:
        #     print(i.shape)
        ...
        ###
        target_size = unrefined_deconvs3_list[0].shape[2:]

        conv_00 = F.interpolate(encoderDict['conv00'], size=target_size, mode='bilinear')

        unrefined_deconvs3_list.append(conv_00)

        # print(len(unrefined_deconvs3_list)) # 3
        # for i in unrefined_deconvs3_list:
        #     print(i.shape)
