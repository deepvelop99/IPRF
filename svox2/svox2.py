...
class SparseGrid(nn.Module):
    ...
    def __add__(self, other):
        if not isinstance(other, SparseGrid):
            raise TypeError("Operands must be of type SparseGrid")
        
        result = SparseGrid()

        # density_data 병합
        result.density_data = torch.cat([self.density_data, other.density_data])

        # sh_data 병합
        result.sh_data = torch.cat([self.sh_data, other.sh_data])
